...
export class LoginComponent implements OnInit {
  ...
  signInWithGoogle(): void {
    this.authService.signIn(GoogleLoginProvider.PROVIDER_ID).then(auth => {
      if (auth) {
        this.user = auth;
        this.setUserInWebStorage(auth.name);
  ...
  signIn(): void {
    ...
    this.dataservice.checkAuth(authInput).subscribe(
      result => {
        this.loaderService.hide();
        if (result) {
          this.setUserInWebStorage(this.loginForm.controls['username'].value);
  ...
  setUserInWebStorage(user): void {
    localStorage.setItem('aiwUserData', user);
  }
